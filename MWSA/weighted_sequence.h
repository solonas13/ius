...
public:
	...
	WeightedSequence(std::string& alphabet, std::vector<std::vector<double>>& p): alph(alphabet), P(p), weighted_index(NULL) {}
    ~WeightedSequence() {
        if (weighted_index) delete weighted_index;
		std::string().swap(alph);
		std::vector<std::vector<double>>().swap(P);
    }
