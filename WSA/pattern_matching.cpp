...
int64_t  lcp ( PropertyString &x, int64_t  M, string & y, int64_t  l, int64_t c)
{
	int64_t  xx = c + x.pi(c);
	if ( M > xx ) return 0;
	int64_t  yy = y.size();
...
pair<int64_t,int64_t> pattern_matching ( string & w, PropertyString & a, int * SA, int * LCP, rmq_succinct_sct<> &rmq, int64_t n )
{
	...
	int64_t N = a.length(); //length of string
	int64_t d = -1;
	int64_t ld = 0;
	int64_t f = n;
	int64_t lf = 0;
	...
	while ( d + 1 < f )
	{
		...
		else
		{
			...
			else if ( ( l == N - SA[i] ) || ( ( SA[i] + l < N ) && ( l != m ) && ( (a[SA[i]+l] < w[l]) || (a.pi(SA[i]) < l) ) ) )
			{
				d = i;
				ld = l;
			}
